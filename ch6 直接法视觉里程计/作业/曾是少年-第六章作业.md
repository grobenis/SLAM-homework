...
1. 切换 I 和 T 的角色后计算误差图像；

2. 步骤3、5和6使用梯度 可以预先计算T的T而不是I的梯度，

3. 式.35 用于计算p而不是等式(10)。 

4. 在将增量扭曲与步骤9中的当前估计值组合之前将其反转。
...
**答**: 窗口大小的选择和误差以及计算量有关系。窗口不能取太大，随着窗口的增大，两帧之间的关键点的计算量将会呈指数增长；窗口可以取单点。但是这样会导致像素梯度计算误差增大.
