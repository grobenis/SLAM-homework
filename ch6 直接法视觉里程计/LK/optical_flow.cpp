...
int main(int argc, char **argv) {
    ...
    cv::imwrite("T_SingleLevel.jpg",img2_single);

    cv::imshow("tracked multi level", img2_multi);
    cv::imwrite("T_MultiLevel.jpg", img2_multi);

    cv::imshow("tracked by opencv", img2_CV);
    cv::imwrite("T_opencv.jpg", img2_CV);
    cv::waitKey(0);
...
void OpticalFlowSingleLevel(
    ...
    for (size_t i = 0; i < kp1.size(); i++) {
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            // 计算代价和雅克比矩阵:compute cost and jacobian
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    if (inverse == false) {
                        ...
                        // error = double(img1.at<uchar>(kp.pt.x + x, kp.pt.y + y)-img2.at<uchar>(kp.pt.x + x + dx , kp.pt.y + y+dy));
